...
axial = mean2(grayImage(row1:row2, col1:col2))
% Compute area
height1 = (row2-row1 + 1);
width1 = (col2 - col1 + 1);
area = height1 * width1;
...
% Get the mean in that box
lateral = mean2(grayImage(row1:row2, col1:col2))
 
%M  MATLAB program on Spatial Resolution

I = imread('200601011218230012.JPG'); %Read image information
subplot(231);
...
z = imread('200601011218230012.JPG');
z=im2gray(z);
z2=imresize(z,0.75);
figure
imshow(z2);
...
I = imread('200601011218230012.JPG');
Igray = rgb2gray(I);
montage({I, Igray});
...
for dd = 1 : length(b)
 ...
 %calculating contrast ratio of an grayscale image
 I = imread('200601011218230012.JPG');
